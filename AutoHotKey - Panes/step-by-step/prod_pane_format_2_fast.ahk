; Power BI – Build/Format pane TOGGLE (Win+2): ultra-fast image-driven
; AutoHotkey v2
#Requires AutoHotkey v2.0
#SingleInstance Force

; ---------- images ----------
ImgPaneIcon := A_ScriptDir "\img\pane_format.png"    ; right-edge icon
ImgTitleRow := A_ScriptDir "\img\title_format.png"   ; "Build/Format" title row

; ---------- hotkey ----------
#3:: TogglePane_ByImages()
...
TogglePane_ByImages() {
    ...
    try {
        ...
        if titleVisible {
            ...
            ; Pane closed -> open via icon, then hit title (no waits)
            if ClickImageInBand(ImgPaneIcon, lIcon,tIcon,rIcon,bIcon, SEARCH_TOLS, CLICK_OffX, CLICK_OffY) {
                ; brief, busy-wait style retries without Sleep
                Loop 8 {
